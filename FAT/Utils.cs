...
namespace FAT
{
    public class Checksummer
    {
        ...
        public static string CheckSumString(Stream stream)
        {
            MD5 Md52 = new MD5CryptoServiceProvider();
            byte[] cs = Md52.ComputeHash(stream);
            BitConverter.IsLittleEndian = true;
            return BitConverter.ToString(cs);
        }
        public static byte[] CheckSumBytes(Stream stream)
        {
            byte[] cs = Md5.ComputeHash(stream);
            return cs;
        }
    }
